...
export function HomepageCategoryShowcase({
  ...
  const eyebrowText = showcase.eyebrow || "Explore Collection";
  const viewAllLabel = showcase.viewAllText || "View All";
  const viewAllLink = showcase.viewAllUrl || `/category/${categorySlug}`;
  const bannerBadgeText = showcase.bannerBadge || "Featured Category";
  const bannerTitleText = showcase.bannerTitle || displayTitle;
