...
export function ManageRibbonsModal({ isOpen, onClose, onRibbonsUpdated }: ManageRibbonsModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-xs p-4 animate-in fade-in duration-150">
      <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl border border-slate-200 dark:border-slate-800 w-full max-w-lg overflow-hidden flex flex-col max-h-[85vh] text-slate-900 dark:text-white">
        {/* Header */}
        <div className="p-5 border-b border-slate-100 dark:border-slate-800 relative">
          ...
          <h2 className="text-lg font-bold text-slate-900 dark:text-white">Manage ribbons for all products</h2>
        ...
        <div className="p-4 overflow-y-auto flex-1 divide-y divide-slate-100 dark:divide-slate-800">
          <div className="flex items-center justify-between pb-2 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
            <span>Ribbon</span>
                  ...
                  {isEditing ? (
                    <div className="flex items-center gap-2 flex-1">
                      ...
                      <button
                        ...
                        className="p-1.5 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 cursor-pointer"
                      >
                        <X className="w-4 h-4" />
                      </button>
                      <button
                        onClick={() => handleSaveEdit(r.id)}
                        className="p-1.5 text-blue-600 dark:text-blue-400 hover:text-blue-700 rounded-full hover:bg-blue-50 dark:hover:bg-blue-900/30 cursor-pointer"
                      >
                    ...
                    <>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <button
                          ...
                          className="px-3 py-1 text-xs font-semibold text-white bg-blue-600 hover:bg-blue-700 rounded-lg transition-colors shadow-xs cursor-pointer"
                        >
                          Edit
                        </button>
                        <button
                          onClick={() => handleDelete(r.id)}
                          className="p-1.5 text-slate-400 hover:text-red-600 dark:hover:text-red-400 rounded-md transition-colors cursor-pointer"
                        >
