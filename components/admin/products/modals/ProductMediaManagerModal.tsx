...
export function ProductMediaManagerModal({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-xs flex items-center justify-center p-4">
      <div
        className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 w-full max-w-4xl max-h-[85vh] flex flex-col shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200 text-slate-900 dark:text-white"
      ...
      >
        ...
        <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-950/50">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-blue-50 dark:bg-blue-900/30 border border-blue-100 dark:border-blue-800/50 flex items-center justify-center text-blue-600 dark:text-blue-400">
              <ImageIcon className="w-5 h-5" />
            </div>
            <div>
              <h3 className="font-bold text-base text-slate-900 dark:text-white">Media Manager</h3>
        ...
        {/* Tab Navigation */}
        <div className="flex items-center border-b border-slate-200 dark:border-slate-800 px-6 bg-white dark:bg-slate-900 gap-2">
          <button
            type="button"
        ...
        {/* Tab Content Area */}
        <div className="flex-1 overflow-y-auto p-6 bg-slate-50/30 dark:bg-slate-950/40">
          ...
          {activeTab === "library" && (
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between gap-3">
                <div className="relative flex-1 max-w-sm">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-3 py-1.5 text-xs bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg focus:outline-hidden focus:ring-2 focus:ring-blue-500 text-slate-900 dark:text-white placeholder-slate-400"
                  />
                </div>

                <div className="flex items-center gap-2 text-xs font-medium text-slate-600 dark:text-slate-400">
                  <span>Selected: <strong className="text-blue-600 dark:text-blue-400 font-bold">{selectedUrls.length}</strong> / {maxSelectable}</span>
                  {selectedUrls.length > 0 && (
                    <button
                      type="button"
                      onClick={() => setSelectedUrls([])}
                      className="text-xs text-red-600 dark:text-red-400 hover:underline ml-2 cursor-pointer"
                    >
                      Clear
                    </button>
                  )}
              ...
              {loading ? (
                ...
                <div className="flex flex-col items-center justify-center py-16 text-center text-slate-400 dark:text-slate-500 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-xl">
                  <ImageIcon className="w-10 h-10 mb-2 stroke-1 text-slate-300 dark:text-slate-600" />
                  <p className="text-sm font-semibold text-slate-600 dark:text-slate-400">No images found</p>
                  <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">Upload an image or switch to the Upload tab.</p>
                </div>
              ) : (
                  ...
                  {filteredImages.map((url, idx) => {
                    ...
                    return (
                      <div
                        ...
                        className={`group relative aspect-square rounded-xl overflow-hidden border cursor-pointer transition-all bg-white dark:bg-slate-950 flex items-center justify-center ${
                          isSelected
                        ...
                        <div
                          className={`absolute top-2 right-2 w-5 h-5 rounded-full flex items-center justify-center transition-all ${
                            isSelected
                              ...
                              : "bg-white/80 dark:bg-slate-900/80 border border-slate-300 dark:border-slate-700 text-transparent group-hover:border-blue-500"
                          }`}
                        >
          ...
          {activeTab === "upload" && (
            <div
              ...
              onDrop={handleDrop}
              className="flex flex-col items-center justify-center py-12 px-6 border-2 border-dashed border-slate-300 dark:border-slate-700 hover:border-blue-500 dark:hover:border-blue-500 rounded-2xl bg-white dark:bg-slate-900 text-center space-y-4 transition-colors"
            >
              <input
                ref={fileInputRef}
                type="file"
              ...
              <div className="w-16 h-16 rounded-full bg-blue-50 dark:bg-blue-900/30 flex items-center justify-center text-blue-600 dark:text-blue-400">
                {isUploading ? (
              ...
              {uploadError && (
                <div className="p-3 bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-400 text-xs font-semibold rounded-xl flex items-center gap-2">
          ...
          {activeTab === "url" && (
            <div className="max-w-xl mx-auto py-8 space-y-4">
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">Image / Media Web Address (URL)</label>
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 px-3.5 py-2 text-xs bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl focus:outline-hidden focus:ring-2 focus:ring-blue-500 font-medium text-slate-900 dark:text-white placeholder-slate-400"
                  />
                  <button
                    type="button"
                ...
                {customUrlError && (
                  <p className="text-xs text-red-600 dark:text-red-400 font-medium">{customUrlError}</p>
                )}
              </div>
              ...
              {customUrlInput.trim().startsWith("http") && (
                <div className="pt-4 border-t border-slate-100 dark:border-slate-800 flex flex-col items-center">
                  <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 mb-2">Live Preview</p>
                  <div className="w-48 h-48 rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden bg-slate-100 dark:bg-slate-950 shadow-xs">
                    <img
                      src={customUrlInput}
                      alt="Preview"
        ...
        <div className="px-6 py-4 border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2 text-xs font-semibold text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white rounded-xl border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors cursor-pointer"
            >
              Cancel
            </button>
